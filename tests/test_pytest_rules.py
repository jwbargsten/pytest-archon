...
from py3arch.collect import collect_from_pkg, collect_from_pkg2
import sys
    ...
    def check(self, package, path=None):
        ...
        for c in candidates:
            ...
            for s in self.required:
                ...
                if not matches:
                    errors.append(f"module {c} did not import anything that matches /{s}/")
            for s in self.forbidden:
                matches = [i for i in imports if re.search(s, i)]
                if matches:
                    errors.append(f"module {c} has forbidden imports {matches} (rule /{s}/)")
        if errors:
